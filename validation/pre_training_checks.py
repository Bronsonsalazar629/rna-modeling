...
def contract_1_label_sequence_alignment(data_dir: Path) -> bool:
    ...
    if issues:
        ...
        print(f"\n✓ PASSED - Checked {len(test_targets)} targets")
...
def contract_2_msa_gap_stripping(data_dir: Path) -> bool:
    ...
    if not msa_dir.exists():
        print("✗ SKIP: MSA directory not found")
        return True
    ...
    if issues:
        ...
        print(f"\n✓ PASSED - Checked {len(msa_files)} MSA files")
...
def contract_3_fape_implementation(tolerance: float = 1e-4) -> bool:
    ...
    if not jnp.allclose(fape_identical, 0.0, atol=tolerance):
        print(f"✗ FAILED: Identical structures FAPE = {fape_identical:.6f} (expected 0.0)")
        return False

    print(f"  ✓ Identical structures: FAPE = {fape_identical:.6f}")
    ...
    print(f"  ✓ Translated structures: FAPE = {fape_translated:.6f}")
    ...
    print(f"  ✓ Known displacement (1 Å): FAPE = {fape_known:.6f}")
    ...
    if not jnp.allclose(fape_ab, fape_ba, atol=tolerance):
        print(f"✗ FAILED: FAPE not symmetric: {fape_ab:.6f} vs {fape_ba:.6f}")
        return False

    print(f"  ✓ Symmetry: FAPE(A,B) = FAPE(B,A)")

    print("\n✓ PASSED - FAPE implementation correct")
    return True
...
def contract_4_equivariance_test() -> bool:
    ...
    if not coords_valid:
        print("✗ FAILED: Coordinates outside valid range")
        return False

    print("\n⚠ PARTIAL - Full equivariance requires rotation-aware input encoding")
    print("  ✓ Coordinates in valid range")
...
def contract_5_gradient_flow() -> bool:
    ...
    try:
        ...
        if not jnp.isfinite(loss_value):
            print(f"✗ FAILED: Loss is not finite: {loss_value}")
            return False

        print(f"  ✓ Loss value: {loss_value:.6f} (finite)")
        ...
        if total_grad_norm < 1e-8:
            print(f"✗ WARNING: Gradient norm very small: {total_grad_norm:.2e}")
